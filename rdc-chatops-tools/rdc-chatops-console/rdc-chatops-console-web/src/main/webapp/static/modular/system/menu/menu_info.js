...
MenuInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/menu/add", function (data) {
        Devops.success("添加成功!");
...
MenuInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/menu/edit", function (data) {
        Devops.success("修改成功!");
        window.parent.Menu.table.refresh();
        MenuInfoDlg.close();
    }, function (data) {
...
MenuInfoDlg.showMenuSelectTree = function () {
    Devops.showInputTree("pcodeName", "pcodeTreeDiv", 15, 34);
};

$(function () {
    Devops.initValidator("menuInfoForm", MenuInfoDlg.validateFields);
