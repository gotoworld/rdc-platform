...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Devops.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            Devops.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            Devops.error("获取详情失败!");
        });
        ajax.start();
...
OptLog.delLog = function () {
    Devops.confirm("是否清空所有日志?",function(){
        var ajax = Devops.baseAjax("/log/delLog","清空日志");
        ajax.start();
