...
DeptInfoDlg.addSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/dept/add", function(data){
        Devops.success("添加成功!");
...
DeptInfoDlg.editSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/dept/update", function(data){
        Devops.success("修改成功!");
        window.parent.Dept.table.refresh();
        DeptInfoDlg.close();
    },function(data){
...
$(function() {
    Devops.initValidator("deptInfoForm", DeptInfoDlg.validateFields);
