...
UserInfoDlg.addSubmit = function () {
    ...
    if (!this.validatePwd()) {
        Devops.error("两次密码输入不一致");
        return;
    }

    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/mgr/add", function (data) {
        Devops.success("添加成功!");
...
UserInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/mgr/edit", function (data) {
        Devops.success("修改成功!");
...
UserInfoDlg.chPwd = function () {
    var ajax = new $ax(Devops.ctxPath + "/mgr/changePwd", function (data) {
        Devops.success("修改成功!");
    }, function (data) {
        Devops.error("修改失败!" + data.responseJSON.message + "!");
    });
...
$(function () {
    Devops.initValidator("userInfoForm", UserInfoDlg.validateFields);
