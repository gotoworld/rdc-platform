...
RolInfoDlg.showDeptSelectTree = function () {
    Devops.showInputTree("deptName", "deptContent");
};
...
RolInfoDlg.showPNameSelectTree = function () {
    Devops.showInputTree("pName", "pNameContent");
};
...
RolInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/role/add", function (data) {
        Devops.success("添加成功!");
...
RolInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/role/edit", function (data) {
        Devops.success("修改成功!");
        window.parent.Role.table.refresh();
        RolInfoDlg.close();
    }, function (data) {
...
$(function () {
    Devops.initValidator("roleInfoForm", RolInfoDlg.validateFields);
