...
NoticeInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/notice/add", function (data) {
        Devops.success("添加成功!");
...
NoticeInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Devops.ctxPath + "/notice/update", function (data) {
        Devops.success("修改成功!");
        window.parent.Notice.table.refresh();
        NoticeInfoDlg.close();
    }, function (data) {
...
$(function () {
    Devops.initValidator("noticeInfoForm", NoticeInfoDlg.validateFields);
