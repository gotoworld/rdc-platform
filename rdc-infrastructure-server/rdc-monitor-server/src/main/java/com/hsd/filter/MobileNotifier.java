...
import com.hsd.config.MonitorMobilePropertiesConfig;
import com.hsd.constant.EnumSmsChannel;
import com.hsd.constant.MqQueueConstant;
import com.hsd.util.template.MobileMsgTemplate;
...
public class MobileNotifier extends AbstractStatusChangeNotifier {
    ...
    protected void doNotify(ClientApplicationEvent event) {
        if (event instanceof ClientApplicationStatusChangedEvent) {
            ...
            String text = String.format("应用:%s 服务ID:%s 下线，时间：%s", event.getApplication().getName(), event.getApplication().getId(), event.getTimestamp());
            rabbitTemplate.convertAndSend(MqQueueConstant.SERVICE_STATUS_CHANGE,
                    new MobileMsgTemplate(StringUtils.join(monitorMobilePropertiesConfig.getUsers().toArray(), ","),
